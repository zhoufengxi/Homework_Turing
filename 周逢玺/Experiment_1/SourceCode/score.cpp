...
printf("----请按以下格式输入学生信息 学号 姓名 平常成绩 实验成绩 期末成绩 \n");
	for (i = 0; i < N; i++)
	{
		scanf_s("%s%s%f%f%f", &SS[i].number, &SS[i].name, &SS[i].dailyScore, &SS[i].expriScore, &SS[i].finalScore);
	}
	i = 0;
	FILE* fp;
	errno_t err;
	...
	for (i; i < N; i++)
	{
		...
		printf("%s %s %3.2f %3.2f %3.2f\n", SS[i].number, SS[i].name, SS[i].dailyScore, SS[i].finalScore, SS[i].generalScore);
...
int analyse()
{
	...
	printf("分段比例:90分以上%4.2f, 80分以上%4.2f, 70分以上%4.2f,  60分以上%4.2f,  不及格%4.2f", j / N, l / N, k / N, n / N, f / N);
	return 0;
}
